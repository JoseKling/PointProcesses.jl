...
function PointProcesses.from_params(::Type{GaussianIntensity{R}}, params::AbstractVector) where {R}
    # Use transformations to ensure positive parameters:
    # params = [log(peak_rate), center, log(width)]
...
function PointProcesses.integrated_intensity(
    ...
    # For Gaussian, we use the error function (analytical, so config is not needed)
    sqrt_2 = sqrt(2.0)
